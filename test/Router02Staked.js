...
contract('Router02 Staked', function (accounts) {
  let root = accounts[0];
  ...
  it('liquidate', async () => {
    ...
    await simpleUniswapOracle.getResult(stakedLPToken.address);
    await expectRevert(
      router.liquidate(
        borrowableUNI.address,
        '0',
  ...
  it('address calculation', async () => {
    ...
    //console.log(await router.getLendingPool(stakedLPToken.address));
    ...
    expect(await router.getBorrowable(stakedLPToken.address, '0')).to.eq(
      expectedBorrowableA
    );
    expect(await router.getBorrowable(stakedLPToken.address, '1')).to.eq(
      expectedBorrowableB
    );
    expect(await router.getCollateral(stakedLPToken.address)).to.eq(
      expectedCollateral
    );
    const lendingPool = await router.getLendingPool(stakedLPToken.address);
