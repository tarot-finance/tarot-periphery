...
async function getPermit(opts) {
  const { token, owner, spender, value, deadline, private_key, borrowPermit } =
    opts;
...
const permitGenerator = {
  ...
  initialize: async () => {
    ...
    for (i = 0; i < 10; i++) {
      ...
      permitGenerator.PKs[borrowerWallet.getAddressString().toLowerCase()] =
        borrowerWallet.getPrivateKey();
    }
  },
  ...
  permit: async (token, owner, spender, value, deadline) => {
    return await permitGenerator._permit(
      token,
      owner,
      spender,
      value,
